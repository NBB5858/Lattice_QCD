...
eval(const uint64_t /*ss*/, T arg) { return arg; }
...
// shoudl stored references to lattice and lattice expressions
template<typename T>
using  wrap = typename std::conditional_t< std::is_arithmetic_v<T> || is_lattice_expr<T>::value, T, const T& >;
