...
public:
    ...
    void hmc_step() {
        ...
        std::complex<double> initial = _integrator.H(phi_tmp, P);
        _integrator.evolve(phi_tmp, P);
        std::complex<double> final = _integrator.H(phi_tmp, P);

        double delta = (final - initial).real();

        double prob_accept = std::min(1.0, std::exp(-delta));
